    ...
    export default {
        ...
        created() {
            if (this.$store.state.table.selectData.nomenklatura) {
                ...
                this.getSelectData('nomenklatura')
                    .then(()=>{
                        this.$emit('loaded')
                        this.stateLoaded = true
                    })
        ...
        computed: {
            ...
            formModel() {
                let fields = [
                    {name:'nomenklatura_id', type:'select',table:'nomenklatura',title:'Компонент',require:true,exceptItems:this.exceptItems},
                    {name:'kolvo', type:'kolvo',title:'Количество',require:true}
                ]
                if (this.val.production_item_id == undefined) {
                    fields.push(
                        {name:'kolvo_per_one', type:'boolean',title:'Указано количество на 1 единицу выпускаемого продукта'}
                    )
                }
                return fields
            },
        ...
        methods: {
            ...
            submit() {
                ...
                if (this.val.with_replace || this.existedNomenklatura.indexOf(this.val.nomenklatura_id)===-1) {
                    this.$emit('input',{
                        ...this.val,
                        ...{
