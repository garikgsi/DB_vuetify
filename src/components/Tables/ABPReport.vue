...
<script>
    ...
    export default {
            ...
            this.getTableModel(this.table)
                .then(()=>{
                    if (this.$store.state.table.filterValues[this.table]) {
                        this.filterValues = {...this.filterValues, ...this.$store.state.table.filterValues[this.table]}
                    }
        ...
        computed: {
            headers() {
                return [
                    {name:'name', title:'Склад', type:'string'}
                ]
            },
            items() {
                try {
            ...
            filters() {
                return [
                    ...
                    {name:'nomenklatura_id', title:'Номенклатура', type:'select', table:'nomenklatura'},
                    {name:'nomenklatura_groups', title:'Группы номенклатур', type:'groups', table:'nomenklatura'},
        ...
        methods: {
            ...mapActions(['getTableData','syncTableOptions','clearTableOptions','getTableModel','setTableFilterValues','getTableFilters']),
            refreshData() {
                this.loading = true
                this.getTableData({'table':this.table})
                    .finally(()=>{
                        this.loading = false
                    })
            },
            ...
            changeFilters(newFilters) {
                this.setTableFilterValues({table:this.table, data:newFilters})
                    .then(()=>{
                        this.refreshData()
                    })
            },
            changeOptions(newOptions) {
                this.syncTableOptions({'table':this.table, 'options':newOptions})
                    .then(()=>{
                        this.refreshData()
                    })
            }
