...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            if (parseInt(this.inputValue)>1) {
                ...
                for (let step=lastStep; step>=0; step--) {
                    ...
                    // получаем данные
                    await this.getData(dataOptions)
                        .then((data)=>{
                            if (val && data[0] != undefined) {
                                ...
                                if (stepTable.key != undefined && item[stepTable.key]!=undefined) {
        ...
        computed: {
            ...
            stepTitle() {
                ...
                if (this.structure[this.step].title !== undefined) {
                    title += `${this.structure[this.step].title} ${this.prevStepValText!=''?`для ${this.prevStepValText}`:``}`
                } else {
            ...
            stepOptions() {
                let prevStep = parseInt(this.step)-1
                if (this.stepKeyField && this.step>0 && this.val[prevStep]!=undefined) {
                    let keyModel = {}
            ...
            // компонент неактивен
            isDisabled() {
                return this.disabled || !this.isDataLoaded
            },
        ...
        methods: {
            ...mapActions(['getForeignSelectData']),
            ...
            getData(options={}) {
                return new Promise((resolve,reject)=>{
                    ...
                    this.getForeignSelectData({table:this.stepTable,options:{...this.stepOptions,...options}})
                        .then((data)=>{
                            newData = data
                            if (data.length==1) {
            ...
            change(newValue) {
                ...
                this.val[this.step] = {id:newValue, title:this.getDataTitle(newValue)}
