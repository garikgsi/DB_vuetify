...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            fileListData() {
                if (this.$store.state.table.fileList[this.table]) {
                    return this.$store.state.table.fileList[this.table].map(item=>{
                        return {id:item.file_id, text:`${item.file.name} (${item.file.extension})`}
                    })
                } else {
                    return null
                }
            ...
            fields() {
                return [
                    {'name':'file_driver_id','type':'radio','items': this.drivers,'title':'Место хранения файла','require':true,'default':3},
                    {'name':'id','type':'integer','title':'id', 'hidden':true},
                    {'name':'file_type_id','type':'int','title':'Тип файла','default':1,'hidden':true},
                    {'name':'name','type':'string','title':'Название','require':true},
                    {'name':'comment','type':'string','title':'Описание','require':false},
                    {'name':'filename', 'type':this.inputFileType, 'title':this.fieldTitle, 'require':true},
                ]
            },
            waitMessage() {
                switch (this.type) {
        ...
        methods: {
            ...
            removeFile(id) {
                switch (this.type) {
                    ...
                    default: {
                        this.deleteFile({table:this.table, id:this.id, file_id:id})
                            .then(()=>{
                                this.getData()
                            })
                    }
            ...
            makeMain(id) {
                this.editFile({table:this.table, id:this.id, file_id:id, values:{is_main:true}})
                    .then(()=>{
                        this.getData()
                    })
            },
...
// console.log(`error=${JSON.stringify(e)}`)
            ...
            syncFileList() {
                if (this.changed) {
                    this.syncFiles({table:this.table, id:this.id, data:this.fileSelector})
                        .then(()=>{
                            this.getData()
                            this.changed = false
                        })
